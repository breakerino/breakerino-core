<?php
// @ripped-from wp-content/plugins/packeta/packetery_vendor/nette/utils/src/Utils/Helpers.php (thanks <3)
...
class Arrays {
	 ...
	 * @param  mixed  $default
	 * @return mixed
	 * @throws \Exception if item does not exist and default value is not provided
	 */
	 ...
	 * @param  string|int|array  $key one or more keys
	 * @return mixed
	 * @throws \Exception if traversed item is not an array
	 */
	public static function &getRef(array &$array, $key) {
		foreach (is_array($key) ? $key : [$key] as $k) {
			if (is_array($array) || $array === null) {
				...
				throw new \Exception('Traversed item is not an array.');
			}
		}
		return $array;
	}
	...
	public static function getKeyOffset(array $array, $key): ?int {
		return array_search(self::toKey($key), array_keys($array), true) || null; // NOTE: Test ($value === false ? null : $value)
	}
	...
	public static function pick(array $array, $keys) {
		...
		foreach ($keys as $key) {
			$pickedArray[$key] = array_key_exists($key, $array) ? $array[$key] : null;
		}
	...
	public static function prefix_keys($array, $prefix) {
		$prefixedArray = [];
	...
	public static function from_entries($arrayOfArrays) {
		return array_reduce($arrayOfArrays, function ($assocArray, $subArray) {
